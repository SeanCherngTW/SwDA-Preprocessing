...
def get_data():
    ...
    for utt in corpus.iter_utterances(display_progress=True):
        no = 'sw{}'.format(str(utt.conversation_no))
        if no in train_set_idx:
            train.append(utt)
        elif no in valid_set_idx:
            valid.append(utt)
        elif no in test_set_idx:
            test.append(utt)
        else:
            others.append(utt)
    print('# utterances: train={}, valid={}, test={}, others={}'.format(len(train), len(valid), len(test), len(others)))
    return train, valid, test, others


def merge_utterance(data):
    conversation = [data[0]]
    conversations = []
    current_no = data[0].conversation_no
    for d in data[1:]:
        if current_no == d.conversation_no:
            conversation.append(d)
        else:
            conversations.append(conversation.copy())
            conversation = [d]
            current_no = d.conversation_no
    ...
    assert total_sent == len(data), '{}, {}'.format(total_sent, len(data))
    return conversations


def merge_plustag(data):
    for i in range(len(data)):
        prevA = None
        prevB = None
        for j in range(len(data[i])):
            current_act = data[i][j].act_tag
            if data[i][j].caller == 'A':
                if current_act == '+':
                    data[i][j].act_tag = prevA
                else:
                    prevA = current_act

            elif data[i][j].caller == 'B':
                if current_act == '+':
                    data[i][j].act_tag = prevB
                else:
                    prevB = current_act
    return data
...
def _map_act(act):
    map_dict = {
        'nn^e': 'ng', 'ny^e': 'na', 'qr': 'qy', 'fe': 'ba', 'fx': 'sv',
        'o': 'fo', 'bc': 'fo', 'by': 'fo', 'fw': 'fo',
        'co': 'oo', 'cc': 'oo',
        'aap': 'aap/am', 'am': 'aap/am', 'arp': 'arp/nd', 'nd': 'arp/nd',
    }

    # Wrong act_tag, change to "sd"
    if not act or act == ':':
        return 'sd'
    if ';' in act:
        act = act[:act.index(';')]
    ...
    # exceptions
    if act in ['qy^d', 'qw^d', 'b^m']:
        return act

    if act in map_dict.keys():
        return map_dict[act]

    if act.startswith('^'):
        act = '^' + act.split('^')[1]
        return map_dict.get(act, act)
    else:
        act = act.split('^')[0]
        return map_dict.get(act, act)
...
def merge_act_tag(data, show_actmap=False):
    from collections import Counter
    act_dict = Counter()
    act_map = {}

    logger = logging.getLogger('Merge Act Tag')

    for i in range(len(data)):
        for j in range(len(data[i])):
            org_act = data[i][j].act_tag
            new_act = _map_act(org_act)
            act_dict[new_act] = act_dict.get(new_act, 0) + 1
            data[i][j].act_tag = new_act
            if org_act not in act_map:
                act_map[org_act] = new_act

    if show_actmap:
        for org, new in act_map.items():
            print('{:5s} -> {:5s}'.format(org, new))
    logger.info(act_dict.most_common())
    logger.info('# unique act: {}\n'.format(len(act_dict.keys())))